...
<head>
    ...
    <style>
        * {
        ...
        .header {
            border-bottom: 3px solid #1a2b5c;
            padding-bottom: 14px;
            margin-bottom: 18px;
            clear: both;
            overflow: hidden;
...
<body>
    ...
    <div class="stats">
        ...
        <div class="stat-box">
            <div class="num" style="color:#92400e">
                {{ $shipments->whereIn('status', ['pending', 'in_transit', 'out_for_delivery'])->count() }}</div>
            <div class="lbl">Proses</div>
        </div>
        <div class="stat-box">
            <div class="num" style="color:#6abf2e">Rp
                {{ number_format($shipments->sum('total_price') / 1000000, 1) }}jt</div>
        ...
        <tbody>
            @foreach ($shipments as $i => $shipment)
                ...
                <tr>
                    ...
                    <td>{{ $shipment->shipment_date ? \Carbon\Carbon::parse($shipment->shipment_date)->format('d/m/Y') : '-' }}
                    </td>
